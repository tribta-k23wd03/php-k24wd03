...
function useForm(rememberKeyOrInitialValues, maybeInitialValues) {
  ...
  const [defaults3, setDefaults] = (0, import_react9.useState)(
  ...
  const [data, setData] = rememberKey ? useRemember(defaults3, `${rememberKey}:data`) : (0, import_react9.useState)(defaults3);
  ...
  const isDirty = (0, import_react9.useMemo)(() => !isEqual(data, defaults3), [data, defaults3]);
      ...
      if (fields.length === 0) {
        ...
        setData(
          (data2) => fields.filter((key) => has(defaults3, key)).reduce(
            (carry, key) => {
              return set2(carry, key, get(defaults3, key));
            },
            { ...data2 }
          )
        );
